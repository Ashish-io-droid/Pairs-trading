...
TARGET_DAILY_VOL = 0.0772451129969544
MAX_LEVERAGE = 3.855438141001019
BUFFER_DAYS = 1200

ENTRY_Z_SCALE = 0.7388195568335812
EXIT_Z_SCALE = 1.6494891095809736

MIN_HOLD_BARS = 6        # minimum bars in a trade before exit allowed
COOLDOWN_BARS = 1        # bars to wait after exit before re-entry

# ---- EXECUTION REALISM (Fixed real-world costs — NOT optimized) ----
# Convention: Signal at close t → Execute at close t (NSE closing auction)
#             PnL = position[t+1] × (spread_close[t+1] - spread_close[t])
#             Costs charged on |Δunits| at the moment of execution
BROKERAGE_BPS      = 3.0     # all-in: brokerage + STT + exchange + GST + stamp duty
SLIPPAGE_BPS       = 2.0     # bid-ask slippage per leg (liquid Nifty Auto stocks ~0.03%)
MARKET_IMPACT_BPS  = 1.0     # market impact per leg (negligible at ₹5L capital)
SHORT_BORROW_RATE  = 0.05    # 5% annual (NSE futures basis ~5-8% for auto stocks)
...
CRISIS_DD_SCALE  = -0.136253852497       # start scaling down exposure
CRISIS_DD_HALT   = -0.2280128325935614   # full halt
...
# ============================================================
# TRADING ENGINE — PRODUCTION EXECUTION SIMULATOR
# ============================================================
# Execution Convention (strictly enforced):
#   Bar t (close):  Observe prices → compute z, allow, vol, crisis
#   Bar t (close):  Signal decision → pos[t]
#   Bar t+1:        Execute via lagged_pos[t+1] = pos[t]
#                   Size via vol_lagged[t+1] = vol[t], cs_lagged[t+1] = cs[t]
#   Bar t+1:        PnL = units[t+1] × ret[t+1]
#   Bar t+1:        Costs = brokerage + slippage(2 legs) + impact(2 legs) + borrow
# ============================================================
def trade_pairs(prices, pair_book, trade_start, pair_pnl_dict, stop_z_allow, stop_z_block,
    ...
    trade_count = {}
    cost_breakdown = {"brokerage": 0.0, "slippage": 0.0, "impact": 0.0, "borrow": 0.0}
    ...
    for a,b,beta,hl in pair_book:
        ...
        # Regime filter (signal component — feeds into pos, which is lagged)
        ...
        # Z-score (signal component — feeds into pos, which is lagged)
        ...
        # Signal decision at close of bar t → pos[t]
        ...
        # ---- EXECUTION (all lagged — uses only bar t-1 data for bar t) ----
        ...
        # ---- PRODUCTION COST MODEL (per-leg, per-trade) ----
        delta_units = np.diff(units, prepend=0)
        abs_traded = np.abs(delta_units)

        # 1. Brokerage: flat rate on traded notional
        brokerage = abs_traded * (BROKERAGE_BPS / 10_000)

        # 2. Slippage: bid-ask crossing cost, charged per leg × 2 legs
        slippage = abs_traded * (SLIPPAGE_BPS * 2 / 10_000)

        # 3. Market impact: price impact from our order flow, per leg × 2 legs
        impact = abs_traded * (MARKET_IMPACT_BPS * 2 / 10_000)

        # 4. Short borrow cost: daily accrual on short leg notional
        #    In a pair trade, ~50% of notional is the short leg
        borrow = np.abs(units) * 0.5 * (SHORT_BORROW_RATE / 252)

        total_cost = brokerage + slippage + impact + borrow

        # PnL = gross return - all execution costs
        pair_pnl = units * ret - total_cost
        pair_pnl = pd.Series(pair_pnl, index=prices.index).fillna(0)

        # Zero out buffer period cleanly (strict index comparison)
        pair_pnl.loc[pair_pnl.index < trade_start] = 0

        # Accumulate cost breakdown (trading window only)
        tw = prices.index >= trade_start
        cost_breakdown["brokerage"] += brokerage[tw].sum()
        cost_breakdown["slippage"]  += slippage[tw].sum()
        cost_breakdown["impact"]    += impact[tw].sum()
        cost_breakdown["borrow"]    += borrow[tw].sum()

        pnl += pair_pnl
        pair_pnl_dict[f"{a}-{b}"] = pair_pnl_dict.get(f"{a}-{b}",0) + pair_pnl.loc[trade_start:].sum()

    return pnl.loc[trade_start:], regime, trade_count, cost_breakdown
...
# BACKTEST
# ============================================================
def run_backtest(stop_z_allow=2.979907622894062, stop_z_block=3.457192773860885):
    ...
    cost_total = {"brokerage": 0.0, "slippage": 0.0, "impact": 0.0, "borrow": 0.0}
    ...
    for i in range(1, len(GLOBAL_REBALANCE)):
        ...
        p, reg, tc, costs = trade_pairs(prices, pairs, start, pair_pnl_dict,
                                 stop_z_allow, stop_z_block,
        ...
        for k in cost_total:
            cost_total[k] += costs[k]

    equity = INITIAL_CAPITAL + pnl.cumsum()
    return equity, pnl, pair_pnl_dict, trade_count_total, regime_total, cost_total
...
# ============================================================
# OPTUNA (legacy — use optimize_fast.py for production tuning)
# ============================================================
def objective(trial):

    global PAIR_LOOKBACK_YEARS, ROLL_Z_WINDOW, VOL_WINDOW
    global TARGET_DAILY_VOL, MAX_LEVERAGE, BROKERAGE_BPS
    ...
    BROKERAGE_BPS = trial.suggest_float("brokerage_bps",3.0,10.0)

    s_allow = trial.suggest_float("stop_allow", 3.0, 5.0)
    s_block = trial.suggest_float("stop_block", 1.5, 3.0)

    equity, pnl, _, _, _, _ = run_backtest(stop_z_allow=s_allow, stop_z_block=s_block)
...
# ============================================================
# EXECUTION — PRODUCTION SIMULATOR
# ============================================================
if __name__ == "__main__":

    equity, pnl, pair_pnl, trade_counts, regime, costs = run_backtest()
    save_outputs(equity, pnl, pair_pnl, trade_counts)

    print("\n" + "=" * 60)
    print("PRODUCTION EXECUTION SIMULATOR — RESULTS")
    print("=" * 60)
